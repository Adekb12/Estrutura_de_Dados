...
class avl {
    ...
    private:
        ...
        void levantamentoAux(noh* umNoh, string local,int &soma);
    public:
        avl() { raiz = NULL; }
        ~avl();
        void imprimir();
...
int avl::levantamento(string local){
    int soma = 0;
    levantamentoAux(raiz,local,soma);
    return soma;
}

void avl::levantamentoAux(noh* umNoh, string local,int &soma){
    if(umNoh != NULL){
        if(umNoh->elemento.localizacao == local)
            soma += umNoh->elemento.quantidadeDisponivel;
        levantamentoAux(umNoh->esq,local,soma);
        levantamentoAux(umNoh->dir,local,soma);   
    }
}
