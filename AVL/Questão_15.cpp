...
class avl {
    ...
    private:
        ...
        void levantamentoAux(noh* umNoh, string marca, int& soma);
    public:
        avl() { raiz = NULL; }
        ~avl();
        void imprimir();
...
int avl::levantamento(string marca){
    int soma = 0;
    levantamentoAux(raiz,marca,soma);
    return soma;
}

void avl::levantamentoAux(noh* umNoh,string marca,int& soma){
    if(umNoh != NULL){
        if(umNoh->elemento.marca == marca)
            soma += umNoh->elemento.quantidadeDisponivel;
        levantamentoAux(umNoh->esq,marca,soma);
        levantamentoAux(umNoh->dir,marca,soma);   
    }
}
