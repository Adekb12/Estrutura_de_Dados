...
class avl {
    ...
    private:
        ...
        void levantamentoAux(noh* umNoh, string cultivar, string  marca,int &soma);
    public:
        avl() { raiz = NULL; }
        ~avl();
        void imprimir();
...
int avl::levantamento(string cultivar, string marca){
    int soma = 0;
    levantamentoAux(raiz,cultivar,marca,soma);
    return soma;
}

void avl::levantamentoAux(noh* umNoh, string cultivar, string  marca,int &soma){
    if(umNoh != NULL){
        if(umNoh->elemento.nomeCultivar == cultivar and umNoh->elemento.marcaSemente == marca)
            soma+=umNoh->elemento.quantidadeDisponivel;
        levantamentoAux(umNoh->esq,cultivar,marca,soma);
        levantamentoAux(umNoh->dir,cultivar,marca,soma);   
    }
}
